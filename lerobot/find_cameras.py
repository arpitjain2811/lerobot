...
def find_all_opencv_cameras() -> List[Dict[str, Any]]:
    """Finds all available OpenCV cameras."""
...
def find_all_realsense_cameras() -> List[Dict[str, Any]]:
    """Finds all available RealSense cameras."""
    ...
    try:
        ...
        logger.warning("Skipping RealSense camera search: pyrealsense2 library not found.")
        return []
    except Exception as e:
...
def find_and_print_cameras(camera_type_filter: str | None = None) -> List[Dict[str, Any]]:
    """
    Finds and prints information about available cameras.

    Args:
        camera_type_filter: Optional filter ("opencv", "realsense", "depthai"). If None, finds all.
    ...
    # Collect cameras based on filter
    if not camera_type_filter or camera_type_filter == "opencv":
        cameras.extend(find_all_opencv_cameras())
    if not camera_type_filter or camera_type_filter == "realsense":
        cameras.extend(find_all_realsense_cameras())
    if not camera_type_filter or camera_type_filter == "depthai":
        cameras.extend(find_all_depthai_cameras())

    # Print results
    if not cameras:
        filter_msg = f" {camera_type_filter}" if camera_type_filter else ""
        logger.warning(f"No{filter_msg} cameras detected.")
    else:
        print("\n--- Detected Cameras ---")
...
def create_camera_instance(cam_meta: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    try:
        # Create camera configuration and instance
        if cam_type == "OpenCV":
            ...
            instance = RealSenseCamera(config)
        ...
        logger.error(f"Failed to connect {cam_type} camera {cam_id}: {e}")
        return None
...
def save_images_from_all_cameras(
    ...
    """
    Connects to cameras and captures images.

    Args:
        output_dir: Directory to save images.
        record_time_s: Duration in seconds to capture images.
        camera_type: Optional filter ("opencv", "realsense", "depthai"). If None, uses all cameras.
    """
